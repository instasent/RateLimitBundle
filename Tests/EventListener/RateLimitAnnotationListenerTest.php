...
namespace Instasent\RateLimitBundle\EventListener\Tests;

use Instasent\RateLimitBundle\Annotation\RateLimit;
use Instasent\RateLimitBundle\EventListener\RateLimitAnnotationListener;
use Instasent\RateLimitBundle\Service\RateLimitService;
use Instasent\RateLimitBundle\Tests\EventListener\MockStorage;
...
class RateLimitAnnotationListenerTest extends TestCase
{
    ...
    public function testRateLimitExpiring()
    {
        ...
        // Expired
        $storage = $this->getMockStorage();
        $storage->createMockRate(':Instasent\RateLimitBundle\EventListener\Tests\MockController:mockAction', 5, -10, 12);
        $listener->onKernelController($event);
        $this->assertInternalType('array', $event->getController());
    }
    ...
    protected function createListener($expects)
    {
        ...
        $this->mockPathLimitProcessor = $this->getMockBuilder('Instasent\RateLimitBundle\Util\PathLimitProcessor')
